  ...
  return (
    <section id="skills" className="py-24 bg-dark-bg bg-grid relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <Reveal delay={100} className="flex items-center justify-start sm:justify-center gap-2 overflow-x-auto pb-4 mb-12 scrollbar-none">
          {categories.map((cat) => (
            <button
              key={cat}
              onClick={() => setActiveCategory(cat)}
              className={`px-4 py-2 rounded-xl text-xs font-mono whitespace-nowrap transition-all duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-ocean-400 ${
                activeCategory === cat
                  ? 'bg-gradient-to-r from-ocean-500 to-sky-500 text-white font-bold shadow-btn-primary border border-ocean-400/40'
                  : 'bg-dark-card border border-dark-border text-slate-400 hover:text-slate-200 hover:border-ocean-400/40 hover:bg-dark-surface'
              }`}
            >
              {cat}
            </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 items-stretch">
          {filteredCategories.map((catGroup, idx) => (
            <Reveal
              ...
              duration={750}
            >
              <div className="glass-card glass-card-hover p-6 rounded-2xl border border-dark-border flex flex-col h-full group">
                {/* Category Header */}
                ...
                <div className="flex flex-wrap gap-2 mt-auto">
                  {catGroup.skills.map((skill, sIdx) => (
                    <div
                      key={sIdx}
                      className="flex items-center justify-between gap-2 px-3 py-2 rounded-xl bg-dark-surface/90 border border-dark-border hover:border-ocean-400/40 hover:bg-ocean-500/5 transition-all duration-150 w-full"
                    >
